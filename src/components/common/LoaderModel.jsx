...
const LoaderModel = ({ open, handleClose, identifier, path }) => {
  const navigate = useNavigate();
  useEffect(() => {
    if (open) {
      const timer = setTimeout(() => {
        if (identifier === "inviteEmployeeTable" && path === "inviteEmployee") {
          navigate("/admin/invite-employees-table");
        }
        if (identifier === "importEmployeeTable" && path === "importEmployee") {
          navigate("/organization-auth/organization-invites");
        }
      }, 1000);
      return () => clearTimeout(timer);
    }
  }, [open, identifier, path, navigate]);
